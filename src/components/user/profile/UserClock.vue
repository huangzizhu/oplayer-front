<template>
  <div id="clock" :class="theme">
    <div class="display">
      ...
      <div class="digits">
        <div :class="digits.h1">
          <span v-for="i in 7" :key="i" :class="`d${i}`"></span>
        </div>
        <div :class="digits.h2">
          <span v-for="i in 7" :key="i" :class="`d${i}`"></span>
        </div>
...
export default {
  ...
  methods: {
    updateTime() {
      ...
      this.digits.h1 = this.digitToName[now[0]];
      this.digits.h2 = this.digitToName[now[1]];
      this.digits.m1 = this.digitToName[now[2]];
      this.digits.m2 = this.digitToName[now[3]];
      this.digits.s1 = this.digitToName[now[4]];
      this.digits.s2 = this.digitToName[now[5]];
      let dow = parseInt(now[6], 10);
      dow--;
      if (dow < 0) dow = 6;
      this.currentDay = dow;
      this.ampm = now[7] + now[8];
...
#clock.dark .alarm {
  background: url("@/assets/images/alarm_dark.jpg");
}
